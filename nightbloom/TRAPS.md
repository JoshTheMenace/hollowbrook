...
## B1 round 2 — the simulation is not reproducible (campaign rules, verbatim)

| Trap | The rule |
|---|---|
| `run.update(dt)` integrated the dash per RENDER frame: realised dash 2.13 m at 30 fps vs 3.20 at 60; one deterministic policy on one seed scored 3180..4100 over six identical runs; a replayed input tape scored 680 vs the 1740 it was recorded from; every gate ran at dt = 1/60 — the single best rate for the greedy bot — and the score window was drawn around it | **Simulation runs on a fixed timestep with an accumulator; render interpolates. Nothing gameplay-relevant integrates per render frame. Gate: an input tape (tick-indexed) replayed at 30/60/90/144 fps and with jittered dt must produce byte-identical state at every checkpoint. Every sim gate states its dt and runs the referee at ≥2 rates.** |
| The drift gate matched 14 source constants while the DELIVERED dash reach was 2.13 m; prose in the contract carried numbers the block did not | **A drift gate must verify DELIVERED values (measured reach, cadence, speed), not only source constants — and it reads the whole contract: numbers in prose must match the block, or prose may not carry numbers the block doesn't.** |
| The ladder gate scored wired parameters; particles rendered at 1–2 px at the play camera contributed nothing a player could see | **A ladder gate judges the COMPOSITE events the game actually fires, measured in rendered space at the play camera. Sub-pixel channels (particles at 1-2 px) contribute nothing to magnitude.** |
| A6 re-derived the greedy window from the greedy bot's own observed spread | **A window derived from the instrument's own spread cannot fail and certifies nothing; windows come from design intent, and a gate tuned on the degenerate policy gates the degenerate policy.** |
...
| A commit-scope guard reported "14 staged paths outside B1 — split the commit" and the commit ran anyway: the guard was piped through `tail -1`, whose exit code (0) replaced the guard's | Never pipe a gate's output when its exit code is the gate: `node guard "scope" >/dev/null && git commit …`. A guard that can be silenced by plumbing is not a guard. The swept files were another project's STAGED work; the series was rewritten and the owner told. |

## B4 round 1 — instrument audit (campaign rules, verbatim)

| Trap | The rule |
|---|---|
| The elite HP bar drew NOTHING: `barFillMat` opaque, `barBackMat` transparent — three.js renders the whole opaque list before the whole transparent list, so `renderOrder` cannot order them and the back plate painted over the fill every frame (pixels identical at 96 % and 14 % HP); it sat in the bundle's own evidence frame | **renderOrder does not order across the opaque/transparent lists.** Keep a bar's plate and fill in the same list (both transparent, or both opaque with polygon offset). And: **every HUD element in a captured evidence frame gets a pixel assertion** (bar fill width vs HP) — the frame-assertion rule extended to UI. |
| Pearson r is invariant under positive scaling: max-normalizing the measured series moved r by exactly zero (0.736002 both ways) and moved MAE alone (0.244 → 0.182); a linear ramp 0→1 PASSES the tracking gate (0.792 / 0.145), the intent curve with its breathers deleted PASSES, a stock unchoreographed survivors run scores r 0.842 / MAE 0.102 — passes both and beats the ride | **Any gate that scores a measured series against a designed curve ships with a null-model table (flat, ramp, noise, the unchoreographed baseline) and the null models must FAIL it. A metric a monotone ramp passes certifies nothing about choreography.** Use per-beat keyframe error, detrended correlation, and an explicit breather depth + timing test. |
| Music was `setIntensity(intent)`: 48 s of drums over ≤ 2 enemies; 33 % of the ride at ≤ 2 enemies; "drive-tier-open" compared three literals from one file and touched no audio | **Music/feel gates compare the layer against measured GAME STATE (threat pressure, enemies near), never against the intent curve alone.** |
| Four amendments, one code commit: the intermediate measurements are unverifiable — "satisfies the letter, forfeits the proof"; substrate and build commits on identical seconds (recurring) | **Each amendment's implementation is its own commit**; substrate and build get their own second. |
| An amendment claimed "no un-telegraphed charger ≥ 80 %" as gated; the gate did not exist | An amendment may not claim a gate that is not in the tree at the commit it cites (the bundle rule, applied to amendments). |
| Elite legibility 1.0 was a tautology: `depthTest:false` marker + dead-bar pixels; the body alone scored `legible:false` | Legibility of a threat is judged on its BODY pixels; a marker is its own row. |
| The novice referee (340 ms) died and the expert (120 ms) lived; the survival cliff sits at 180–240 ms — inside the human band, sampled by neither profile | Sample the referee's reaction axis densely enough to locate the cliff; two profiles bracket nothing. |
| The referee picked upgrade cards at random: ±3/6 survival dispersion against a ≥ 4/6 window | A referee's build choice is a deliberate policy, or build choice is declared part of the skill axis. |
| Headroom by damage per minute survived read 2.00×; total damage read 1.32× | Report both when a normalisation can flatter; the reviewer picks. |
