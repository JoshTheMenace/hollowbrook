...
`experiments/skill-context-tests/quality-loop/medieval-river-town/spatial-contract-findings.md`).
...
"terrain": {
  ...
  "shelves": [                       // optional: flat sub-regions INSIDE one district
    { "in": "crag", "x0": 26, "z0": -6, "x1": 39, "z1": 4, "y": 4.0, "tone": "bank" },
    { "in": "crag", "x0": 32, "z0": -4, "x1": 38, "z1": 2, "y": 6.4, "tone": "bank" }
  ],
  "mounds": [                        // optional: smooth domes on top of that field
    { "x": 33, "z": 0, "rx": 11, "rz": 10, "h": 5.6, "tone": "bank" }
  ],
  "crossings": [                     // one entry per socket PAIR…
    ...
    // …and, with no `socket`, an INTERNAL climb inside one district
    { "id": "crag-bench-steps", "in": "crag", "kind": "stairs",
      "at": [21.38, -2], "axis": "x", "width": 3, "from": 1.6, "to": 4.0, "rise": 0.22 }
  ],
...
- **Shelves** — a flat rect *inside* one district's envelope, held at its own
  height. One level per district is all the ground a flat district needs and
  none of what a district with real landform needs, and since terrain-first
  forbids it laying its own, without shelves such a district must either be
  flat or break the rule. Measured, the district with genuine landform scored
  the **highest** of four, so "be flat" is not an acceptable answer. A shelf
  joins the levels in `levelAt`'s max-over-containing-rects — it simply wins
  where it applies — and **every shelf edge becomes a grid line**, exactly as
  an envelope edge does, or the 6.0 m it was promised is rounded off a cell
  that half-covers the ground beside it. Shelves may overlap; higher wins.
- **Mounds** — a smooth elliptical dome *added* on top of the level/shelf
  field, for landform that is not terraced: a rock, a knoll, a spur. It is the
  only thing here that is added rather than declared, so it is the only thing
  that can move a height somebody was promised — and therefore it is
  multiplied by a keep-out mask that is **exactly zero inside every shelf,
  every crossing corridor and every tread**, and for 0.6 m outside them,
  reaching full strength 2.2 m further out. `smoothstep` returns a hard 0
  below its lower bound, so that is exactness by construction rather than by
  tolerance: measured over 1 900 samples inside two shelves with a 5.6 m dome
  standing round them, the worst lift is **0.000000 m**. District *levels* are
  deliberately not masked — a mound that could not lift a district's own
  ground would be a mound with nowhere to stand — so an anchor under one has
  to sit on a shelf or expect the lifted height, and the plan gate warns with
  the number. Mounds also subdivide the lattice (a cell over one may fall at
  most 0.30 m, sized from the dome's own peak gradient) and those lines run
  the full width of the city, so keep them few and generous in radius.
- **Crossings** — the terrain builds **both halves** of every socket crossing:
  ...
  A crossing with **no `socket`** is an *internal* climb instead — the same
  ramp or flight between two heights inside one district, reusing the whole
  code path (landings, the going clamp, tread rects skipped by the grid,
  platform registration). That is how a district climbs its own rock without
  laying ground: the shelf is terrain, the flight up to it is terrain, and the
  district dresses both. `at` is the **foot** — the point on the centreline
  where the run begins at `from` — running in the `+axis` direction unless
  `dir: -1` says otherwise, and the gate checks the run and the width against
  the envelope it is inside.
...
Two techniques carry it, both proven on `experiments/harbor-town`'s headland,
...
- **A conforming grid.** Grid lines go in at the footprint edges, at every
  envelope edge and every **shelf** edge (plus a hairline either side, so a
  level change draws as a vertical face instead of ramping across a whole
  cell), at every crossing rect edge and at every stair tread edge — and
  *only then* is each gap
