...
export function cel(opts = {}) {
  ...
  // NOTE: `flat` is NOT passed to the material.  three.js ignores flatShading on
  // MeshToonMaterial and warns once per material -- it was a no-op inherited
  // from the reference scene, which looks correctly faceted anyway because
  // FACETING IS A PROPERTY OF THE GEOMETRY, not of the material: a merged,
  // non-indexed geometry with computeVertexNormals() already shades flat.  Use
  // faceted(geometry) below when a smooth-normalled geometry needs hard facets.
...
/**
 * Hard facets for a geometry that has smooth normals.  This is the real lever:
 * the material flag does nothing on a toon material, and everything the bake
 * merges is non-indexed anyway, which is already flat.
 */
