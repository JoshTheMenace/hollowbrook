 ...
 */
// VIGNETTE DEFAULT ONLY -- OVERRIDDEN BY THE PLAN.  If the scene exports a
// plan with city.compass, the block further down re-aims sun and fill from
// city.sun (bearing + elevation) and resizes the shadow cascade and the fog to
// city.footprint_m.  Read the RUNTIME value, not this constant: a district
// agent on the first city built this way art-directed against these numbers
// and reported a 45-degree sun when the plan's own sun was 18.
const RIG = {
...
const legibility = createLegibilityCheck({
  renderer,
  scene,
  camera,
  pipeline,
  cameras: vignette.reviewCameras,
  onResize: (w, h) => setOutlineResolution(w, h),
});
...
if (vignette.plan?.city?.compass) {
  ...
  /* The far plane is the plan's, not the vignette's.  At 110 m the arrival
   * vista's own declared subject stood at 110.2 m — clipped by the plane,
   * and then hazed by a fogRange that clamps itself against it.  Derive it
   * from where the vista cameras actually stand. */
  camera.far = Math.max(camera.far, cameraFar(spec, vignette.plan.vista_cameras ?? []));
  camera.updateProjectionMatrix();
  const fogr = fogRange(spec.footprint_m, camera.far);
  scene.fog.near = fogr.near;
  scene.fog.far = fogr.far;
  // the polish pass's p4 overrides these on its first tick; keep the
  // unpolished range so the legibility gate can measure the vistas BOTH
  // ways and say whether the haze is costing a declared subject.
  unpolishedFog = { near: fogr.near, far: fogr.far };
...
function frame() {
  ...
  // the eye drives distance-culled interiors and auto-opening doors
  // (core/interior.js); every other updater takes dt and ignores it
  vignette.update(dt, camera.position);
...
window.__vignette = {
  ...
  /* vista LEGIBILITY gate: checkAllCameras asks whether a ray reaches the
   * subject; this asks whether the subject READS — its pixel share of the
   * frame and its luma against the ring around it, measured in the graded
   * frame.  All three vistas passed the raycast gate while two of them
   * failed pictorially, which is what this exists for. */
  checkVistaLegibility: legibility.checkVistaLegibility,
  checkAllVistas: legibility.checkAllVistas,
  /* The runtime half of the polish toggle.  p1-p3 are geometry and are
   * decided at build time by `?polish=off`; p4 is the fog range and can be
   * switched here, so `checkAllVistas({ polish: setPolishHaze })` measures
   * every vista with and without the aerial perspective. */
  setPolishHaze: (on) => {
    if (!unpolishedFog) return;
  ...
  checkSpatial: createSpatialCheck({
    ...
    // enterable buildings: without these the seam grid only ever sees the
    // roof over a room, so a missing interior floor passes silently
