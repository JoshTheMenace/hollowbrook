import { cached, surfaceTex } from './texkit.js';
...
/* THE FACES THESE MAPS LAND ON, in metres, MEASURED off the built scene
 * rather than remembered — traverse it, group meshes by `material.map`,
 * and take the largest face dimension per map.  A pooled map is sized for
 * the largest face in its pool and tiles by `repeat` on the smaller ones;
 * `texelsFor` turns each number below into a power-of-two canvas at
 * TEXELS_PER_M (48 px/m).  Restate these when the kit's building scale
 * changes: a constant that stopped being true when the world grew is the
 * most expensive kind of bug this project has. */
const FACE = {
  plaster: 8,    // a render/limestone gable
  rubble: 12,    // a tower or a revetment elevation
  roof: 12,      // a pantile pitch — the biggest single card in the kit
  tar: 8,        // a boarded flank
};
...
export function plasterTex() {
  return surfaceTex('polish-plaster', FACE.plaster, (c, w, h) => {
    c.fillStyle = '#fdfcfa';
    c.fillRect(0, 0, w, h);
    const r = rng('plaster');
...
export function rubbleTex() {
  return surfaceTex('polish-rubble', FACE.rubble, (c, w, h) => {
    c.fillStyle = '#fcfbf9';
    c.fillRect(0, 0, w, h);
    const r = rng('rubble');
...
export function roofTex() {
  return surfaceTex('polish-roof', FACE.roof, (c, w, h) => {
    c.fillStyle = '#fdfcfb';
    c.fillRect(0, 0, w, h);
    const r = rng('roof');
...
export function tarTex() {
  return surfaceTex('polish-tar', FACE.tar, (c, w, h) => {
    c.fillStyle = '#fbfbfb';
    c.fillRect(0, 0, w, h);
    const r = rng('tar');
...
export function litGlassTex() {
  /* NOT a surface map: this one lands on a single pane or a lantern body,
   * a metre at most, so 128 px is already ~128 px/m and sizing it by face
   * area would spend memory on nothing. */
