...
export function createCameraCheck({ scene, cameras, colliders = [], footprintHeight = Infinity, aspect = 16 / 9 }) {
  ...
  /* An INTERIOR camera stands in a distance-culled room, and three has
   * changed its mind about whether a raycast hits an invisible mesh.  So
   * every camera check runs with interiors forced visible and restores
   * after: a check is not the runtime.  (It also matters for a camera in
   * the STREET whose subject is glimpsed through a door.) */
  const checkCamera = (name) => withInteriorsVisible(scene, () => checkCameraNow(name));

  function checkCameraNow(name) {
    ...
    if (!view.subject || !String(view.subject).trim()) {
      ...
      if (!subject) {
        ...
        // Aim at the centre AND at 80% height: for a tall landmark (a tower,
        // a bellcote) the part that must read is its upper reach, and the
        // centre is mid-shaft behind whatever canopy or roofline stands
        // between.  The city gate's landmark check has always tried both;
        // this check disagreeing with it cost a round of diagnosis.
        const top = centre.clone(); top.y = box.max.y - (box.max.y - box.min.y) * 0.2;
        const dir = centre.clone().sub(pos);
        if (dir.lengthSq() < 1e-6) {
          failures.push(`camera sits exactly on subject "${view.subject}" — back it off`);
        } else {
          let blocked = null;
          for (const aim of [top, centre]) {
            const hit = firstHit(new THREE.Raycaster(pos, aim.clone().sub(pos).normalize()));
            if (!hit) { blocked = null; break; }
            if (inSubtree(hit.object, subject)) { blocked = null; break; }
            const slack = box.clampPoint(hit.point, new THREE.Vector3()).distanceTo(hit.point);
            if (slack <= SUBJECT_SLACK_M) { blocked = null; break; }
            blocked = `view of subject "${view.subject}" is blocked by "${label(hit.object)}" ${hit.distance.toFixed(2)} m from the camera (${slack.toFixed(2)} m short of the subject)`;
          }
