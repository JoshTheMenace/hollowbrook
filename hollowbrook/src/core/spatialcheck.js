...
import { withInteriorsVisible } from './interior.js';

/* ------------------------------------------------------------------ *
 * Spatial audit.
 *
 * Screenshots miss metre-scale embedding and floating (measured in
 * experiments/skill-context-tests/quality-loop/medieval-river-town): a
 ...
 *
 * ENTERABLE BUILDINGS (see core/interior.js) need two things from this
 * file, and both are small:
 *
 *   - INTERIORS ARE FORCED VISIBLE for the length of the audit.  The
 *     down-cast index skips `!mesh.visible`, and an interior's dressing
 *     is distance-culled at runtime -- so an audit run with the camera
 *     out in the street indexes NONE of the furniture's supporting floor
 *     and reports every prop in the room as floating in open air.
 *     `withInteriorsVisible` wraps the whole check here rather than in
 *     each script, so every caller inherits it.
 *   - `interiorFloors` teaches the SEAM GRID to look inside.  It casts
 *     down from y = 30, and over a building the first surface it meets is
 *     the ROOF -- which is well above the claimed walkable height, so a
 *     room with no floor at all passes silently.  A sample inside a
 *     registered room drops its ray from just under that room's ceiling
 *     instead, which is the only way a hole in an interior floor is found.
 *     `hollowShell` registers the rects; scene.js passes
 *     `ctx.interiorFloors` through.
 * ------------------------------------------------------------------ */
...
function buildDowncast(scene) {
  ...
  /* `visible` is inherited by the renderer, so the skip has to walk the
   * ancestors too: a group turned off (a distance-culled interior) leaves
   * every mesh inside it with `visible === true` of its own, and a
   * per-mesh test therefore indexes geometry that is not drawn.  With
   * this, the interiors guard below is load-bearing rather than merely
   * defensive. */
  const drawn = (o) => { for (let n = o; n; n = n.parent) if (n.visible === false) return false; return true; };
  scene.traverse((mesh) => {
    if (!mesh.isMesh || !mesh.geometry?.attributes?.position || !drawn(mesh)) return;
...
export function createSpatialCheck({ scene, groundAt = null, colliders = [], footprint = null, islandSets = null, linearSets = null, interiorFloors = [], holeFloorY = HOLE_FLOOR_Y }) {
  /* Hollowbrook: the sunk market's level is -1.4, under the audit's fixed
   * -0.5 hole floor — 1 952 samples of perfectly good paving reported as a
   * hole.  The caller passes the terrain's lowest level minus 0.5
   * (scene.js exports it as `holeFloorY`); the default is the old constant. */
  function runSpatial() {
    ...
    if (footprint) {
      ...
      for (let x = rect.x0 + GRID_M / 2; x < rect.x1; x += GRID_M) {
        for (let z = rect.z0 + GRID_M / 2; z < rect.z1; z += GRID_M) {
          ...
          samples += 1;
          /* Inside a registered room the ray must start UNDER the ceiling:
           * from y = 30 the first surface over a building is its roof, so
           * an interior with no floor at all is a silent pass. */
          const room = interiorFloors.find((r) => x > r.x0 && x < r.x1 && z > r.z0 && z < r.z1);
          if (room) inside += 1;
          const hit = cast.down(x, z, room ? room.ceilY - 0.05 : SEAM_FROM_Y);
          if (!hit || hit.y < holeFloorY) {
    ...
    const stats = {
      ...
      seamSamplesInInteriors: inside,
      interiors: interiorFloors.length,
    ...
    const header = `spatial audit: ${stats.units} units (${stats.tagged} tagged, ${stats.clusters} pooled clusters, ${stats.runs} runs, ${stats.rakedRuns} of them raked), ${stats.triangles} triangles, ${stats.seamSamples} seam samples (${stats.seamSkippedInColliders} inside colliders skipped` +
      `${interiorFloors.length ? `, ${inside} inside ${interiorFloors.length} registered interior${interiorFloors.length === 1 ? '' : 's'}` : ''})`;
    const lines = [header];
  ...
  /* Culling is a runtime optimisation; a gate is not the runtime.  Every
   * interior is forced visible for the length of the audit and restored
   * after — see the header. */
  const checkSpatial = () => withInteriorsVisible(scene, runSpatial);
