...
export function composeCity({ plan, districts, ctx, terrainMaterials = null, only = null }) {
  ...
  for (const id of order) {
    ...
    wrapped.platform = (x0, z0, x1, z1, top, ...rest) => {
      ...
      /* An INTERIOR floor is exempt: the defect this warning exists for is
       * a ground plate that stops at the envelope with nobody owning what
       * lies beyond it, and a floor inside four walls cannot have it. */
      if (area > GROUND_PLATFORM_M2 && !(rest[0] && rest[0].interior)) {
        const [cx, cz] = rectCenter(Math.min(x0, x1), Math.min(z0, z1), Math.max(x0, x1), Math.max(z0, z1));
