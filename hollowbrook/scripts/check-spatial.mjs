...
try {
  ...
  const { checkSpatial } = createSpatialCheck({
    ...
    // enterable buildings: the seam grid casts from above everything, so
    // over a room it lands on the roof and a missing floor is a silent
    // pass. checkSpatial also forces every interior visible for the run.
