...
export function build() {
  ...
  const skinShade = toonMaterial('#d8a377');       // cheek/vertex-shaded face uses vc instead
  const hakamaFlat = toonMaterial('#43424c');
  const obiM = toonMaterial('#4a3f4d');
  const scarfM = toonMaterial('#c9403a', { rim: 0.45 });
  ...
  const gold = toonMaterial('#c9a13f', { rim: 0.5 });
  const cream = toonMaterial('#e2d6bc', { rim: 0.3 });
  const dark = toonMaterial('#2a2622', { rim: 0 });
  const white = toonMaterial('#f7f4ee', { rim: 0.2 });
  const amber = toonMaterial('#e8a13c', { rim: 0.25 });
  const hairFlat = toonMaterial('#2c3a6e', { rim: 0.35 });
  const sandalM = toonMaterial('#a08050');
  ...
  const face = grad(ball(0.185, vc, [1, 1.02, 0.95]), '#d8a377', '#f6d0ab');
  ...
  const hairGrad = (mesh) => grad(mesh, '#222c52', '#6d82c4');
  ...
  const ring = grad(facet(ringGeo, vc), '#8f2b27', '#e05a4d');
  ...
  const knotBall = grad(facetBall(0.052, vc, [1.1, 1.0, 0.9], [6, 5]), '#8f2b27', '#e05a4d');
  knotBall.position.set(0.005, -0.005, -0.005);
  scarfA.add(knotBall);
  const segA = grad(facetBall(0.058, vc, [0.95, 1.85, 0.55], [7, 5]), '#8f2b27', '#e05a4d');
  ...
  const segB = grad(facetBall(0.050, vc, [0.9, 1.85, 0.5], [7, 5]), '#7e2622', '#d14e42');
  segB.position.set(0.008, -0.055, -0.012);
  segB.rotation.x = D(-10);
  scarfB.add(segB);
  const tipSeg = grad(facetBall(0.038, vc, [0.8, 1.6, 0.5], [6, 4]), '#7e2622', '#d14e42');
